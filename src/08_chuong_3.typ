...
Thuật toán tìm kiếm lân cận sử dụng các phép biến hình cây TBR trên một cây $T$ với cạnh cắt
$R$ được thực hiện như sau:
- Chọn cạnh $R$ là cạnh cắt của các phép TBR cần khảo sát. Trong trường hợp tổng quát, cắt cây
  $T$ tại $R$ tạo ra hai cây con $T_1$, $T_2$ và cạnh $R$ tạm thời tách biệt với nhau. Cạnh $R$ sau đó sẽ được dùng làm cạnh trung gian để nối $T_1$ và $T_2$.

- Xét lần lượt các cặp cạnh nối ($I_1$, $I_2$) với $I_1$ thuộc cây con $T_1$, $I_2$ thuộc cây con $T_2$ và
  khoảng cách giữa $I_1$, $I_2$ ở trên cây ban đầu nằm trong khoảng $["mintrav", "maxtrav"]$ cho trước.
  - Thực hiện nối hai cạnh $I_1$ và $I_2$ thông qua cạnh $R$. (xem #ref(label("tbr-example"))C)
  - Cây kết quả nhận được là cây $T^*$. Tính toán, đánh giá cây $T^*$ thông qua điểm parsimony. Cập
    nhật cây lân cận tốt nhất tìm được $T^"best"$ .
  - Thực hiện cắt cạnh $R$ một lần nữa, nhằm khảo sát những cặp ($I_1$, $I_2$) tiếp theo.
- Sau khi tìm kiếm kết thúc, ta sẽ tìm được cây $T^"best"$ tốt nhất khi thực hiện các phép TBR
  trên cây $T$ với cạnh cắt $R$.
- Nối lại cạnh $R$ vào vị trí ban đầu, rollback về cây $T$ ban đầu.

Thuật toán được mô tả bằng mã giả trong @algo1 sau đây. Khi đó, với cây $T$, cạnh $R$, giá trị $"mintrav"$ và $"maxtrav"$ sau khi thực hiện thủ tục #smallcaps("ComputeTBR")$(R, "mintrav", "maxtrav")$ sẽ tìm được cây $T^"best"$ (tương đương với tìm được cặp cạnh tốt nhất ($I_1$, $I_2$)).

Những trường hợp đặc biệt như $R$ không phải là cạnh trong (nối với một đỉnh lá) được xử lí riêng do công đoạn cắt cạnh và nối cạnh có phần khác biệt. Ngoài ra, để tính nhanh điểm $"MP"(T^*)$, ta cũng sẽ đổi cạnh gốc của cây $T^"lst"$ thành chính cạnh cắt $R$ sau lượt #smallcaps("TestTBRMove") đầu tiên và giữ nguyên cho tới khi xét cạnh cắt tiếp theo. Điều này đảm bảo số lượng đỉnh cần phải tính lại sẽ không quá $O("maxtrav")$ đỉnh (không xét lượt thử đầu tiên do gốc có thể khác $R$).
#outline_algo(
    [
        #algo(header: [
        #table(
        ...
        ], strong-keywords: false, indent-guides: 1pt + gray, breakable: true)[ 
        #smallcaps("ComputeTBR")$(R, "mintrav", "maxtrav")$\
        \
        *Function* #smallcaps("TestTBRMove")$(I_1,I_2)$#i\
        #text(fill: rgb("#7d6f6f"),[\/\/ $T$ is already cut into $R$, $T_1$, $T_2$ when called #smallcaps("TestTBRMove()")])\
        ...
        *Function* #smallcaps("ComputeTBR")$(R, "mintrav", "maxtrav")$#i\
        ...
        #text(fill: rgb("#7d6f6f"),[\/\/  Find all valid $(I_1,I_2)$ can be done recursively via DFS])\
...
Chúng tôi cũng đề xuất một thuật toán tìm kiếm lân cận TBR tương tự @algo1 nhưng với một số thay đổi nhỏ. Ở thuật toán trên, với mỗi cạnh cắt $R$, ta cập nhật cây hiện tại tối đa 1 lần (nếu như cây $T^"best"$ cho kết quả tốt hơn). Ở thuật toán thay đổi này (xem @algo2), với mỗi cặp cạnh cắt $R$ và cạnh nối $I_1$ ta cập nhật cây hiện tại tối đa 1 lần. Chi tiết hơn, ta sẽ xét mọi cạnh nối $I_2$ thỏa mãn, tìm cây $T^"best"$ và cập nhật cho cây hiện tại nếu cho kết quả tốt hơn.
...
Thuật toán leo đồi TBR thực hiện leo đồi cập nhật cây $T$ bằng cây $T^"best"$ tìm được (nếu $T^"best"$ cho kết quả tốt hơn) với mỗi cạnh cắt $R$ khảo sát bằng thuật toán được mô tả ở @algo1 và @algo2. Vòng lặp leo đồi sẽ tiếp tục trong khi cây $T$ vẫn được cập nhật bởi một cây tối ưu hơn. Thuật toán leo đồi TBR được mô tả bằng mã giả trong @algo3 sau đây.

Khi leo đồi sử dụng hai cách tìm kiếm lân cận TBR khác nhau (sau đây gọi tắt là hai cách leo đồi "tốt nhất" và "tốt hơn") thì mẫu không gian cây khảo sát được cũng khác nhau.
...
Chúng tôi đề xuất MPBoot-TBR (xem @algo4) bằng cách thay thế toàn bộ leo đồi SPR bằng leo đồi TBR. Nếu một lượt lặp tìm kiếm ở pha 2 không tìm được một cây có điểm số MP thấp hơn so với điểm số của $T^"best"$ , thì lượt lặp sẽ được coi là unsuccessful (thất bại). Thuật toán duy trì biến $n_"unsuccess"$ lưu số lượt lặp tìm kiếm liên tiếp unsuccessful (thất bại). MPBoot gốc dừng nếu $n_"unsuccess"$ đạt $n^'$ (giá trị làm tròn lên tới số hàng trăm gần nhất của $n$). Do tập lân cận của TBR lớn hơn, chúng tôi hiệu chỉnh giới hạn của $n_"unsuccess"$ thành $n^'=100$ giống với IQ-TREE @minh2020iq.
...
#outline_algo(
    [
        ...
        #table(
            ...
            table.hline(stroke: 0.5pt),
            [*Output*], [A tree $T^"best"$ with best found $"MP"(T^"best" | A^"data")$ and a set $cal(B)$ of bootstrap trees ${T_1, T_2, dots, T_B}$],
            table.hline(stroke: 0.5pt), [], []
        )
        ], strong-keywords: false, indent-guides: 1pt + gray, breakable: true)[ 
        *Phase 1: Initialization*#i\
            ...
            Initialize the candidate set $C$ for $A^"data"$ with 100 random stepwise addition procedures followed by TBR hill-climbing.#d\
        \
        *Phase 2: Exploration*#i\
            *do*#i\
                Improve $C$ by performing perturbation on a randomly selected tree from the candidate set $C$ and a subsequent TBR hill-climbing step.\
