...
Tiêu chí Maximum Parsimony (MP) được sử dụng để xây dựng cây tiến hóa bằng cách tối thiểu hóa chi phí thay thế cần thiết để giải thích các chuỗi trong một phân tích đa chuỗi (MSA). Điểm MP của một cây được tính bằng cách cộng tổng điểm của từng vị trí (cột) trong MSA.

Bài toán xây dựng cây bootstrap tiến hóa (Phylogenetic Bootstrapping) (nguồn gốc từ phương pháp bootstrap trong thống kê @efron1992bootstrap) liên quan đến việc tạo ra nhiều bản sao bootstrap của MSA gốc bằng cách lấy mẫu các vị trí với sự thay thế. Mục tiêu là xác định cây tốt nhất cho cả MSA gốc và mỗi bản sao bootstrap, theo tiêu chí MP. Kết quả được tóm tắt dưới dạng tần suất phân chia nhánh, được sử dụng để đánh giá sự hỗ trợ cho mỗi phần của cây, thể hiện qua giá trị hỗ trợ bootstrap. Xây dựng cây bootstrap tiến hóa theo tiêu chuẩn Maximum Parsimony là bài toán tối ưu tổ hợp thuộc lớp NP-complete @graham1982unlikelihood. 

MPBoot @hoang2018mpboot là một công cụ phần mềm được thiết kế để thực hiện phân tích bootstrap MP một cách hiệu quả. MPBoot sử dụng các phép thao tác sắp xếp lại cây như Nearest Neighbor Interchange (NNI) và Subtree Pruning and Regrafting (SPR) để tìm kiếm cây tốt nhất. Tuy nhiên, MPBoot chủ yếu dựa vào SPR, điều này có thể hạn chế hiệu suất trong việc khám phá không gian cây.

== Mục tiêu của đề tài

Xây dựng cây bootstrap tiến hóa theo tiêu chí maximum parsimony là một bài toán NP-complete @graham1982unlikelihood, khiến thuật toán leo đồi trở thành lựa chọn phổ biến. MPBoot hiện sử dụng leo đồi để cải thiện lời giải thông qua các kỹ thuật biến đổi cây như Nearest Neighbor Interchange (NNI) và Subtree Pruning and Regrafting (SPR). Tuy nhiên, phép biến đổi Tree Bisection and Reconnection (TBR), vốn mạnh mẽ hơn, vẫn chưa được tích hợp vào MPBoot. Mục tiêu của khóa luận này là tích hợp TBR vào MPBoot, đồng thời phát triển nhiều tính năng mới như checkpoint, hỗ trợ dữ liệu khác như morphology, binary, dữ liệu lớn, và cải tiến các tính năng hiện có để phát triển phiên bản MPBoot2. Để khai thác tối đa sức mạnh của các phép biến đổi, khóa luận còn phát triển MPBoot-RL, sử dụng giải thuật đàn kiến (Ant Colony Optimization) để kết hợp linh hoạt các phép biến đổi, nhằm tối ưu hóa cả điểm số và thời gian tính toán. Đồng thời, khóa luận đề xuất một ánh xạ độ khó của dữ liệu dựa trên quá trình tối ưu của thuật toán.

== Cấu trúc của khóa luận

Phần còn lại của khóa luận này được trình bày như sau:
- @chuong2: Giới thiệu các khái niệm cơ sở về bài toán xây dựng cây bootstrap tiến hóa và giải thuật đàn kiến.
...
- @chuong6: Kết luận về các thuật toán đề xuất và kết quả thực nghiệm, đồng thời chỉ ra các định hướng cải tiến trong tương lai.
