...
Tập hợp các cây bootstrap $cal(B)$ thường được tóm tắt dưới dạng một vectơ tần suất của các
...
Phương pháp bootstrap chuẩn xây dựng tập cây $cal(B)$ thông qua $B$ lần chạy thuật toán tìm kiếm cây độc lập với mỗi $A_b$. Các công trình sử dụng phương pháp này bao gồm TNT @goloboff2023tnt, PAUP\* @swofford2003phylogeny, MEGA @tamura2007mega4. Vì phương pháp bootstrap chuẩn có xu hướng đánh giá thấp khả năng đúng của một phân hoạch nhị phân @hillis1993empirical@minh2013ultrafast, nên quy tắc thực hành phổ biến là coi các cạnh có giá trị hỗ trợ bootstrap hơn 70% là đáng tin cậy. 
==== Phương pháp MPBoot

Phương pháp MPBoot sử dụng tiêu chuẩn maximum parsimony (với ưu điểm là tính đơn giản, dễ cài đặt và hiệu quả trong thiết kế cấu trúc dữ liệu) cùng với phương pháp xấp xỉ bootstrap để giải quyết bài toán xây dựng cây bootstrap tiến hóa. Phương pháp xấp xỉ bootstrap trong MPBoot xác định tập hợp $cal(B)$ bằng cách thực hiện một lần tìm kiếm cây trong không gian cây biểu diễn sắp hàng gốc $A^"data"$ (xem @approx-boot). 
...
trong đó $tau_(x y)$ là lượng pheromone được để lại khi chuyển từ trạng thái $x$ đến trạng thái $y$, $alpha gt.eq 0$ là tham số điều khiển ảnh hưởng của $tau_(x y)$, $eta_(x y)$ là mức độ "hấp dẫn" khi chuyển trạng thái từ $x$ đến $y$ (thường được gọi là thông tin heuristic) và $cal(B) gt.eq 1$ là tham số điều khiển ảnh hưởng của $eta_(x y)$. $tau_(x z)$ và $eta_(x z)$ đại diện cho mức độ pheromone và sự hấp dẫn đối với các cách chuyển trạng thái khác có thể có.
...
Thuật toán được đề cập ở trên là thuật toán ACO đầu tiên, có tên gọi là thuật toán Ant System (AS) @colorni1991distributed.
...
Trong thuật toán này, giải pháp tốt nhất toàn cục sẽ để lại pheromone trên dấu vết của nó sau mỗi vòng lặp (ngay cả khi dấu vết này không được quay lại), cùng với tất cả các con kiến khác. Chiến lược ưu tú có mục tiêu chỉ đạo quá trình tìm kiếm của tất cả các con kiến để xây dựng một giải pháp chứa các liên kết của tuyến đường tốt nhất hiện tại.

==== Thuật toán Max–Min Ant System (MMAS)

Thuật toán này kiểm soát lượng pheromone tối đa và tối thiểu trên mỗi dấu vết. Chỉ có chuyến đi tốt nhất toàn cục hoặc chuyến đi tốt nhất trong vòng lặp mới được phép thêm pheromone vào dấu vết của nó. Để tránh sự trì trệ trong thuật toán tìm kiếm, phạm vi lượng pheromone có thể có trên mỗi dấu vết bị giới hạn trong một khoảng $[tau_"max",tau_"min"]$. Tất cả các cạnh đều được khởi tạo với $tau_"max"$ để thúc đẩy việc khám phá các giải pháp cao hơn. Các dấu vết sẽ được khởi tạo lại với $tau_"max"$ khi gần đến mức trì trệ.

==== Thuật toán Rank-based ant system (ASrank)

Tất cả các giải pháp đều được xếp hạng theo chiều dài của chúng. Chỉ có một số lượng con kiến tốt nhất trong vòng lặp này mới được phép cập nhật dấu vết của chúng. Lượng feromone được lưu lại sẽ được cân nhắc cho từng giải pháp, sao cho các giải pháp có đường đi ngắn hơn sẽ lưu lại nhiều feromone hơn các giải pháp có đường đi dài hơn.

==== Thuật toán đàn kiến song song (PACO)

Do các cá thể kiến trong thuật toán rất độc lập với nhau nên các cá thể kiến có thể được chia thành các nhóm và chạy đồng thời trên các bộ xử lý khác nhau. Điều này giúp tăng hiệu suất của thuật toán, tuy nhiên cần có các phương pháp trao đổi thông tin vết mùi pheromone giữa các nhóm một cách hiệu quả. Có một số phương pháp trao đổi vết mùi thông dụng như all-to-all, directed/undirected ring, hypercube, random,...

==== Thuật toán Max-Min trơn (SMMAS)

Một cải tiến so với giải thuật đàn kiến gốc là giải thuật đàn kiến Max-Min (MMAS), nơi đặt giới hạn trên và dưới cho các giá trị mùi nhằm thúc đẩy việc khám phá và tránh sự hội tụ sớm. Xây dựng trên MMAS, giải thuật đàn kiến Max-Min trơn (SMMA) @do2008pheromone tích hợp một cơ chế làm mượt điều chỉnh vào quy tắc cập nhật mùi nhân tạo. Yếu tố làm mượt này giúp điều chỉnh tốc độ thay đổi của mùi nhân tạo, chuyển từ việc tập trung vào việc khám phá vào giai đoạn đầu của tìm kiếm đến việc khai thác mạnh mẽ hơn khi thuật toán hội tụ.
